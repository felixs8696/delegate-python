...
pip install git+ssh://git@github.com/stainless-sdks/delegate-python.git
```
...
Simply import `AsyncDelegate` instead of `Delegate` and use `await` with each API call:
...
```sh
# install from this staging repo
pip install 'delegate[aiohttp] @ git+ssh://git@github.com/stainless-sdks/delegate-python.git'
```
...
These methods return an [`APIResponse`](https://github.com/stainless-sdks/delegate-python/tree/main/src/delegate/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/delegate-python/tree/main/src/delegate/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/delegate-python/issues) with questions, bugs, or suggestions.
